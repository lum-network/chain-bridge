        ...
        for (const validator of validators) {
            ...
            while (true) {
                ...
                for (const delegation of delegations.delegationResponses) {
                    await this._validatorDelegationService.createOrUpdate(delegation.delegation.delegatorAddress, delegation.delegation.validatorAddress, parseInt(delegation.delegation.shares, 10) / CLIENT_PRECISION, {
                    ...
                    for (const val of stakingValidators.validators) {
                        ...
                        for (let v = 0; v < validators.length; v++) {
                            if (validators[v].consensus_pubkey === consensus_pubkey) {
                                ...
                                validators[v].delegator_shares = parseInt(val.delegatorShares, 10) / CLIENT_PRECISION;
