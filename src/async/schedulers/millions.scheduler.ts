...
export class MillionsScheduler {
    ...
    async poolsSync() {
        if (!this._configService.get<boolean>('MILLIONS_POOLS_SYNC_ENABLED')) {
            return;
        }

        try {
            this._logger.log(`Syncing pools from chain...`);
            const pools = await this._chainService.getChain(AssetSymbol.LUM).client.queryClient.millions.pools();
            ...
            for (const pool of pools) {
                ...
                const formattedPool: Partial<MillionsPoolsEntity> = {
                    pool_id: pool.poolId.toString(),
                    denom: pool.denom,
                    native_denom: pool.nativeDenom,
                    ...
                    next_draw_id: pool.nextDrawId.toString(),
                    tvl_amount: pool.tvlAmount,
                    depositors_count: pool.depositorsCount.toString(),
                    last_draw_state: pool.lastDrawState,
                    state: pool.state,
                    created_at_height: pool.createdAtHeight.toString(),
                    updated_at_height: pool.updatedAtHeight.toString(),
                ...
                await this._millionsPoolsService.createOrUpdate(formattedPool);
            }
        } catch (error) {
            this._logger.error(`Failed to ingest pools: ${error}`);
        }
    }
}
