...
export default class ValidatorScheduler {
    ...
    async ingest() {
        const vss: { height; result: { block_height; validators: [] } } = await BlockchainService.getInstance()
            .getClient()
            .getValidatorsSet();
        const vs: { height; result: [] } = await BlockchainService.getInstance()
            .getClient()
            .getValidators();
        ...
        for (const set of vss.result.validators) {
            for (const val of vs.result) {
                ...
                if (
                    (await ElasticService.getInstance().documentExists(
                        ElasticIndexes.INDEX_VALIDATORS,
                        set['address'],
                    )) === false
                ) {
                    await ElasticService.getInstance().documentCreate(
                        ElasticIndexes.INDEX_VALIDATORS,
                        set['address'],
                        payload,
                    );
