...
export class SentinelService {
    ...
    getApy = async (): Promise<number> => {
        try {
            const metrics = await computeApyMetrics(this.sentinelClient, Number(await this.getTokenSupply()), CLIENT_PRECISION, TEN_EXPONENT_SIX);
    ...
    getTokenInfo = async (): Promise<TokenInfo> => {
        try {
            ...
            return {
                name: DfractAssetName.SENTINEL,
                symbol: DfractAssetSymbol.SENTINEL,
                ...getTokenInfo,
            };
        } catch (error) {
