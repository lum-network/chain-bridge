...
export default class ValidatorsController {
    ...
    async fetch() {
        ...
        const bonded = await BlockchainService.getInstance()
            .getClient()
            .getValidators('bonded');
        const unbonded = await BlockchainService.getInstance()
            .getClient()
    ...
    async show(@Req() req: Request) {
        ...
        const validator = await BlockchainService.getInstance()
            .getClient()
            .getValidator(req.params.address);
        if (!validator) {
        ...
        const delegations = await BlockchainService.getInstance()
            .getClient()
            .getValidatorDelegations(req.params.address);
        if (!delegations) {
        ...
        const rewards = await BlockchainService.getInstance()
            .getClient()
            .getValidatorOutstandingRewards(req.params.address);
        if (!rewards) {
