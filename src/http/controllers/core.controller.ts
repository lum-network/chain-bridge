...
export class CoreController {
    ...
    async params(): Promise<DataResponse> {
        const [chainId, mintingParams, stakingParams, govDepositParams, govVoteParams, govTallyParams, distributionParams, slashingParams, communityPoolParams] = await Promise.all([
            ...
            this._chainService.getChain(AssetSymbol.LUM).client.cosmos.gov.v1beta1.params({ paramsType: 'deposit' }),
            this._chainService.getChain(AssetSymbol.LUM).client.cosmos.gov.v1beta1.params({ paramsType: 'voting' }),
            this._chainService.getChain(AssetSymbol.LUM).client.cosmos.gov.v1beta1.params({ paramsType: 'tallying' }),
